...
def parser(id):
    try:
        ...
        try:
            parf = driver.find_element(By.XPATH,"//aside//child::a").click()     
            item = driver.find_element(By.XPATH,"//span[contains(@class,'tsBodyL')]") #and "//span[contains(@class,'tsBodyL')]/following::span//font").text == "Парфюмер"
            item.click()
        except:
        ...
        try:
            ...
            try:
                ...
                pass
        # парсит массив доп ссылок фото в строку и подставляет  точку запятую ,вместо  запятой    
            print(linkUrldop)
            #linkUrldop.append(bigDopFoto)
    ...
    #ключевые  слова
        ...
        print('Возможно отсутствует связь с интернетом')
    i=i
    return id,name,price,priceSkidka,nds,KomercialType,articul,i
...
def addFoto():
    ...
    # берем токен  из  приложения в яндекс почты
        ...
        try: 
            ...
            print('файл уже существует')
    '''
    #загружает фото на яндекс диск с паки компа ,делает публичную ссылку ,заносит в переменую
        try: 
            y.upload(f"C:\\ozon1\\image1\\{result}",f"/image/{result}")
        ...
        print('фото не удалось добавить')
